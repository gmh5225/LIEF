...
Binary::Binary(const std::string &name, PE_TYPE type) : type_{type}
{
    ...
    name_ = name;
    Header &hdr = header();
    size_t sizeof_headers = dos_header().addressof_new_exeheader() + sizeof(details::pe_header) +
...
template <typename T>
static void
write_impl(Binary &binary, T &&dest)
{
    Builder builder{binary};

    builder.build_imports(false).patch_imports(false).build_relocations(false).build_tls(false).build_resources(true);
...
void
Binary::write(const std::string &filename)
{
    write_impl(*this, filename);
}

void
Binary::write(std::ostream &os)
{
    write_impl(*this, os);
}
...
uint64_t
Binary::va_to_offset(uint64_t VA)
{
...
uint64_t
Binary::imagebase() const
{
    return optional_header().imagebase();
}

result<uint64_t>
Binary::offset_to_virtual_address(uint64_t offset, uint64_t slide) const
{
    const auto it_section =
        std::find_if(std::begin(sections_), std::end(sections_), [offset](const std::unique_ptr<Section> &section) {
            return (offset >= section->offset() && offset < (section->offset() + section->sizeof_raw_data()));
        });
...
uint64_t
Binary::rva_to_offset(uint64_t RVA)
{
    const auto it_section =
        std::find_if(std::begin(sections_), std::end(sections_), [RVA](const std::unique_ptr<Section> &section) {
            const auto vsize_adj = std::max<uint64_t>(section->virtual_size(), section->sizeof_raw_data());
            return section->virtual_address() <= RVA && RVA < (section->virtual_address() + vsize_adj);
        });
...
const Section *
Binary::section_from_offset(uint64_t offset) const
{
    const auto it_section =
        std::find_if(std::begin(sections_), std::end(sections_), [&offset](const std::unique_ptr<Section> &section) {
...
Section *
Binary::section_from_offset(uint64_t offset)
{
    return const_cast<Section *>(static_cast<const Binary *>(this)->section_from_offset(offset));
}

const Section *
Binary::section_from_rva(uint64_t virtual_address) const
{
    const auto it_section = std::find_if(
        std::begin(sections_), std::end(sections_), [virtual_address](const std::unique_ptr<Section> &section) {
...
bool
Binary::has(DATA_DIRECTORY index) const
{
    const auto it = std::find_if(
        std::begin(data_directories_), std::end(data_directories_), [index](const std::unique_ptr<DataDirectory> &d) {
            return d->type() == index;
        });
...
bool
Binary::has_rich_header() const
{
    return has_rich_header_;
}

bool
Binary::has_tls() const
{
    return has_tls_;
}

bool
Binary::has_imports() const
{
    return has_imports_;
}

bool
Binary::has_signatures() const
{
    return !signatures_.empty();
}

bool
Binary::has_exports() const
{
    return has_exports_;
}

bool
Binary::has_resources() const
{
    return has_resources_ && resources_ != nullptr;
}

bool
Binary::has_exceptions() const
{
    return has(DATA_DIRECTORY::EXCEPTION_TABLE);
}

bool
Binary::has_relocations() const
{
    return has_relocations_;
}

bool
Binary::has_debug() const
{
    return has_debug_;
}

bool
Binary::is_reproducible_build() const
{
    return is_reproducible_build_;
}

bool
Binary::has_configuration() const
{
    return has_configuration_ && load_configuration_ != nullptr;
}

const LoadConfiguration *
Binary::load_configuration() const
{
...
Binary::it_const_sections
Binary::sections() const
{
    return sections_;
}

LIEF::Binary::sections_t
Binary::get_abstract_sections()
{
    LIEF::Binary::sections_t secs;
    secs.reserve(sections_.size());
    std::transform(
        std::begin(sections_), std::end(sections_), std::back_inserter(secs), [](const std::unique_ptr<Section> &s) {
            return s.get();
        });
    return secs;
}
...
Section *
Binary::get_section(const std::string &name)
{
    return const_cast<Section *>(static_cast<const Binary *>(this)->get_section(name));
}

const Section *
Binary::get_section(const std::string &name) const
{
    const auto section_it =
        std::find_if(std::begin(sections_), std::end(sections_), [&name](const std::unique_ptr<Section> &section) {
...
void
Binary::remove_section(const std::string &name, bool clear)
{
...
void
Binary::remove(const Section &section, bool clear)
{
    const auto it_section =
        std::find_if(std::begin(sections_), std::end(sections_), [&section](const std::unique_ptr<Section> &s) {
            return *s == section;
        });
...
void
Binary::make_space_for_new_section()
{
...
Binary::add_section(const Section &section, PE_SECTION_TYPES type, SECTION_CHARACTERISTICS characteristic)
{
    ...
    const auto it_section =
        std::find_if(std::begin(sections_), std::end(sections_), [&type](const std::unique_ptr<Section> &s) {
    ...
    // Set 'nop' to new part
    memset(content.data() + section_size, 0x90, section_size_aligned - section_size);
    ...
    uint64_t new_section_offset = align(
        std::accumulate(
            std::begin(sections_),
            std::end(sections_),
            static_cast<uint64_t>(sizeof_headers()),
    ...
    const uint64_t new_section_va = align(
        std::accumulate(
            std::begin(sections_),
            std::end(sections_),
            section_align,
...
Binary::it_relocations
Binary::relocations()
{
    return relocations_;
}

Binary::it_const_relocations
Binary::relocations() const
{
    return relocations_;
}

Relocation &
Binary::add_relocation(const Relocation &relocation)
{
...
void
Binary::remove_all_relocations()
{
    relocations_.clear();
}

LIEF::Binary::relocations_t
Binary::get_abstract_relocations()
{
...
ImportEntry *
Binary::add_import_function(const std::string &library, const std::string &function)
{
    const auto it_import = std::find_if(std::begin(imports_), std::end(imports_), [&library](const Import &import) {
...
Import &
Binary::add_library(const std::string &name)
{
...
void
Binary::remove_library(const std::string &)
{
...
uint32_t
Binary::predict_function_rva(const std::string &library, const std::string &function)
{
    const auto it_import = std::find_if(
        imports_.cbegin(), imports_.cend(), [&library](const Import &imp) { return imp.name() == library; });
    ...
    size_t nb_functions = std::count_if(entries.cbegin(), entries.cend(), [&function](const ImportEntry &entry) {
    ...
    uint32_t import_table_size =
        static_cast<uint32_t>((imports().size() + 1) * sizeof(details::pe_import)); // +1 for the null entry
    ...
    for (auto it_imp = imports_.cbegin(); (it_imp->name() != library && it_imp != imports_.cend()); ++it_imp)
    {
        if (type_ == PE_TYPE::PE32)
        {
    ...
    for (auto it_func = entries.cbegin(); (it_func->name() != function && it_func != entries.cend()); ++it_func)
    {
        if (type_ == PE_TYPE::PE32)
        {
    ...
    const uint64_t section_align = static_cast<uint64_t>(optional_header().section_alignment());
    const uint64_t next_virtual_address = align(
        std::accumulate(
            std::begin(sections_),
            std::end(sections_),
            section_align,
...
bool
Binary::has_import(const std::string &import_name) const
{
    return get_import(import_name) != nullptr;
}

Import *
Binary::get_import(const std::string &import_name)
{
    return const_cast<Import *>(static_cast<const Binary *>(this)->get_import(import_name));
}

const Import *
Binary::get_import(const std::string &import_name) const
{
    const auto it_import = std::find_if(std::begin(imports_), std::end(imports_), [&import_name](const Import &import) {
...
void
Binary::set_resources(const ResourceDirectory &resource)
{
    resources_ = std::make_unique<ResourceDirectory>(resource);
}

void
Binary::set_resources(const ResourceData &resource)
{
...
//
/////////////////////////////////////
Binary::it_data_directories
Binary::data_directories()
{
    return data_directories_;
}

Binary::it_const_data_directories
Binary::data_directories() const
{
    return data_directories_;
}

Binary::debug_entries_t &
Binary::debug()
{
    return const_cast<debug_entries_t &>(static_cast<const Binary *>(this)->debug());
}

const Binary::debug_entries_t &
Binary::debug() const
{
    return debug_;
}
...
Binary::it_const_signatures
Binary::signatures() const
{
    return signatures_;
}

std::vector<uint8_t>
Binary::authentihash(ALGORITHMS algo) const
{
    ...
    // Sort by file offset
    std::sort(std::begin(sections), std::end(sections), [](const Section *lhs, const Section *rhs) {
    ...
    for (const Section *sec : sections)
    {
        ...
        LIEF_DEBUG(
            "Authentihash:  Append section {:<8}: [0x{:04x}, 0x{:04x}] + [0x{:04x}] = [0x{:04x}, 0x{:04x}]",
            sec->name(),
            sec->offset(),
            sec->offset() + content.size(),
            pad.size(),
            sec->offset(),
...
Signature::VERIFICATION_FLAGS
Binary::verify_signature(const Signature &sig, Signature::VERIFICATION_CHECKS checks) const
{
    Signature::VERIFICATION_FLAGS flags = Signature::VERIFICATION_FLAGS::OK;
...
Binary::symbols() const
{
    return symbols_;
}

LIEF::Binary::functions_t
Binary::get_abstract_exported_functions() const
{
...
LIEF::Binary::functions_t
Binary::get_abstract_imported_functions() const
{
    LIEF::Binary::functions_t result;
    ...
    for (const DelayImport &import : delay_imports())
    {
        for (const DelayImportEntry &entry : import.entries())
        {
            ...
            if (!name.empty())
            {
                result.emplace_back(name, entry.value(), Function::flags_list_t{Function::FLAGS::IMPORTED});
            }
        }
    }
    return result;
...
Binary::get_abstract_imported_libraries() const
{
...
LIEF::Header
Binary::get_abstract_header() const
{
    LIEF::Header header;
...
void
Binary::hook_function(const std::string &function, uint64_t address)
{
...
void
Binary::hook_function(const std::string &library, const std::string &function, uint64_t address)
{
...
// ==============
uint64_t
Binary::entrypoint() const
{
    return optional_header().imagebase() + optional_header().addressof_entrypoint();
}

void
Binary::patch_address(uint64_t address, const std::vector<uint8_t> &patch_value, LIEF::Binary::VA_TYPES addr_type)
{
    ...
    {
        LIEF_ERR(
            "The patch value ({} bytes @0x{:x}) is out of bounds of the section (limit: 0x{:x})",
            patch_value.size(),
            offset,
            content.size());
        return;
    }
    std::copy(std::begin(patch_value), std::end(patch_value), content.data() + offset);
}

void
Binary::patch_address(uint64_t address, uint64_t patch_value, size_t size, LIEF::Binary::VA_TYPES addr_type)
{
...
std::vector<uint8_t>
Binary::get_content_from_virtual_address(uint64_t virtual_address, uint64_t size, LIEF::Binary::VA_TYPES addr_type)
    const
{
...
void
Binary::dos_stub(const std::vector<uint8_t> &content)
{
    dos_stub_ = content;
}
...
const RichHeader &
Binary::rich_header() const
{
    return rich_header_;
}

void
Binary::rich_header(const RichHeader &rich_header)
{
...
Binary::resources_manager() const
{
    ...
    if (has_tls())
    {
        ...
        for (size_t i = 0; i < clbs.size(); ++i)
        {
            functions.emplace_back(
                "tls_" + std::to_string(i), clbs[i], Function::flags_list_t{Function::FLAGS::CONSTRUCTOR});
        }
    }
    return functions;
}
...
LIEF::Binary::functions_t
Binary::functions() const
{
    static const auto func_cmd = [](const Function &lhs, const Function &rhs) { return lhs.address() < rhs.address(); };
    ...
    std::move(std::begin(exported), std::end(exported), std::inserter(functions_set, std::end(functions_set)));

    std::move(std::begin(ctors), std::end(ctors), std::inserter(functions_set, std::end(functions_set)));

    return {std::begin(functions_set), std::end(functions_set)};
}

LIEF::Binary::functions_t
Binary::exception_functions() const
{
    LIEF::Binary::functions_t functions;
    if (!has_exceptions())
    {
...
// ========================================================
bool
Binary::has_delay_imports() const
{
    return !delay_imports_.empty();
}

Binary::it_delay_imports
Binary::delay_imports()
{
    return delay_imports_;
}

Binary::it_const_delay_imports
Binary::delay_imports() const
{
    return delay_imports_;
}

DelayImport *
Binary::get_delay_import(const std::string &import_name)
{
    return const_cast<DelayImport *>(static_cast<const Binary *>(this)->get_delay_import(import_name));
}

const DelayImport *
Binary::get_delay_import(const std::string &import_name) const
{
    const auto it_import =
        std::find_if(std::begin(delay_imports_), std::end(delay_imports_), [&import_name](const DelayImport &import) {
            return import.name() == import_name;
        });
...
bool
Binary::has_delay_import(const std::string &import_name) const
{
    return get_delay_import(import_name) != nullptr;
}

void
Binary::accept(Visitor &visitor) const
{
    visitor.visit(*this);
}
...
std::ostream &
Binary::print(std::ostream &os) const
{
